...
| Nieuwe Nor: 30 events € 3 (locker "3 euro" in de FAQ onder elk event), veel 'gratis' ("kans op gratis tickets" in de footer) en een uur te vroeg in de winter (JSON-LD met vaste "+02:00") | Bedragen met locker/garderobe/munt/parkeer/bijdrage-context zijn geen prijs; 'gratis' telt niet bij winacties, parkeren, wifi, garderobe. Een offset +01:00/+02:00 is een Nederlandse kloktijd en wordt letterlijk genomen (`to_local`); alleen echte UTC (Z/+00:00) wordt omgerekend. |
...
| Fixtures-test op 55 echte pagina's (6 sep) vond zes leesfouten | Bitterzoet "15 euro 25+" (leeftijd als prijs); Iduna "€ 19,95 vvk, € 24,95 deur" (label ná het bedrag); Baroeg "augustus 20, 2026" (WordPress-postdatum in F j, Y-formaat vóór de eventdatum) en een lege eerste h1; Nobel `<time datetime=publicatietijd>zaterdag 07 november</time>` (tekst wint van een tijdstempel; alleen-verleden tags tellen niet); Simplon "kaartverkoop start donderdag 25 juni" (verkoopstart is geen eventdatum); Doornroosje/Merleyn JSON-LD "+00:00" die lokale tijd bleek (gelabelde aanvang precies 1–2 uur eerder wint); EKKO `<main>` als fragment (hele pagina lezen als main < 50%). Ticketshop-url's worden niet meer als eventpagina gelezen (`enrich_from_detail`), want die tekst bevat álle shop-events. |
